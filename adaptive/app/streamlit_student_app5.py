...
#                     return prereq_qs.sample(1).iloc[0], f"⚠️ Let's review a core concept: **{prereq}**"
...
#     if not eligible_concepts:
#         return None, "🎉 **Mastery Reached!** High proficiency in all concepts."
...
#         return candidate_qs.sort_values('bloom_level').iloc[0], f"🧠 Working on **{target_concept}** (Mastery: {knowledge[target_concept]:.0%})"
...
#             return question.iloc[0], "🔁 Retrying a previously missed question."
...
#     st.sidebar.title("📚 Java Practice")
#     selected_topic = st.sidebar.selectbox("📘 Choose a Topic", topics)
#     # ... (sidebar controls) ...
...
#             st.success("✅ Correct!")
#         else:
#             st.error(f"❌ Incorrect. The correct answer was **{q['correct_option'].upper()}**.")
...
#             st.session_state.current_question = q.to_dict()
#             st.session_state.current_reason = "🔖 Reviewing bookmarked question."
#         else:
...
#     if col2.button("📎 Bookmark Question"):
#         st.session_state.bookmarked.add(q['question_id'])
...
#     st.title("👩‍🏫 Teacher Dashboard")
#     st.subheader("📊 Question Distribution by Topic and Bloom's Level")
    
#     if not df.empty:
#         dist = df.groupby(["topic", "bloom_level"]).size().unstack().fillna(0)
#         st.bar_chart(dist)

#     st.subheader("👨‍🎓 Student Session Log")
...
#         #     st.markdown(f"🧾 *Hint/Explanation:* {q['explanation']}")
            
#         # if pd.notna(q["main_explanation"]):
#         #     st.markdown(f"🧾 *Hint/Explanation:* {q['main_explanation']}")
...
#             submit = st.button("✅ Submit Answer")
#         with col2:
#             bookmark = st.button("📎 Bookmark Question")
...
#                 # ❓
#                 with st.expander("🗸"):
#                     # st.markdown(f"🧾 Correct answer: **{correct.upper()}**\n*Explanation:* {q['main_explanation']}")
#                     st.markdown(f"🧾 *Correct answer:* **{correct.upper()}**", unsafe_allow_html=True)
