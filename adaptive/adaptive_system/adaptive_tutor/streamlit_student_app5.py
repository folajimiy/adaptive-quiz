...
def initialize_session_state():
    defaults = {
        ...
        "wrong_qs": [],
        "confidence_record": {},
        "topic_mastery": {},         # per-topic Bloom mastery points
        "topic_mastery_status": {},  # per-topic status: Not started / In progress / Mastered
        "asked_qs": set(),
        "submitted": False,
        ...
        "selected_mode": None,       # "Practice" or "Test"
        "session_done": False,
        "question_count": 0,
        "question_start_time": None  # for response time measurement
    }
...
def render_sidebar(topics):
    ...
    # Student view mode (Dashboard vs Practice/Test)
    if role == "Student":
        view_mode = st.sidebar.radio(
            "Student view",
            ["Dashboard", "Practice/Test"],
            key="student_view_mode_radio"
        )
        st.session_state.student_view_mode = view_mode
    else:
        st.session_state.student_view_mode = "Practice/Test"

    # Topic selection (still needed for both dashboard + practice)
...
def render_student_dashboard(df):
    """
    Student Home Dashboard / Profile View.
    Shows:
      - overall mastery
      - per-topic status
      - progress rings (simple)
    ...
    if df_logs.empty:
        ...
        st.info("No activity yet. Start a Practice or Test session to see progress here.")
        return
    else:
        total = len(df_logs)
    ...
    # --- Mastery by Topic (accuracy) ---
    st.markdown("### 📈 Mastery by Topic")

    topic_acc = df_logs.groupby("topic")["is_correct"].mean() * 100
    topic_acc_df = topic_acc.sort_values(ascending=False).to_frame("Accuracy (%)")
    st.bar_chart(topic_acc_df)

    # --- Progress rings for top 3 topics ---
    st.markdown("### 🎯 Focus Topics (Progress Rings)")

    top_topics = topic_acc.sort_values(ascending=False).head(3)
    cols = st.columns(len(top_topics) if len(top_topics) > 0 else 1)
    ...
    # Friendly hint
    st.caption("Tip: Use Practice Mode to build mastery, then Test Mode to check your readiness.")
...
def render_student_view(df):
    ...
    # --- Load student info / level from CSV ---
    if "student_level" not in st.session_state:
        try:
            ...
            if student_id:
                ...
                st.session_state.student_level = level_series.iloc[0] if not level_series.empty else "Beginner"
            else:
    ...
    # --- Initialize Bloom level based on level ---
    starting_bloom = {
        "Beginner": "Remember",
        "Intermediate": "Apply",
        "Advanced": "Evaluate"
    ...
    # --- Sidebar: role, view mode, topic selection ---
    role, selected_topic = render_sidebar(
        [t for t in df['topic'].dropna().unique() if t in allowed_topics]
    )
    if selected_topic is None:
        return

    # NEW: If student chose "Dashboard", render dashboard and stop
    if st.session_state.get("student_view_mode", "Practice/Test") == "Dashboard":
        render_student_dashboard(df)
        return

    # ---------- from here down, keep the modernized Practice/Test UI ----------
    st.markdown(
        """
        <div style="padding: 0.5rem 0 1rem 0;">
          <h1 style="margin-bottom: 0.2rem;">🎓 Adaptive Java Tutor</h1>
          <p style="color: #555; font-size: 0.95rem;">
            Practice Java concepts with adaptive questions, Bloom-based progression, and mastery tracking.
          </p>
        </div>
        """,
        unsafe_allow_html=True
    ...
    if st.session_state.selected_mode is None:
        st.info("Select how you want to work on this topic today.")
    ...
    with c1:
        st.markdown(
            """
            <div style="border-radius: 0.75rem; padding: 0.75rem; border: 1px solid #e5e7eb;">
              <b>🧪 Test Mode</b><br/>
              • Fixed-length quiz<br/>
              • No hints or explanations until the end<br/>
              • Good for pre/post tests
            </div>
            """,
            unsafe_allow_html=True
        )
    with c2:
        st.markdown(
            """
            <div style="border-radius: 0.75rem; padding: 0.75rem; border: 1px solid #e5e7eb;">
              <b>🧠 Practice Mode</b><br/>
              • Fully adaptive & explanatory<br/>
              • Review incorrect questions<br/>
              • Bookmarks & mastery feedback
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    st.markdown(
        f"### 📘 Topic: {selected_topic} &nbsp;&nbsp;|&nbsp;&nbsp; Level: **{student_level}** &nbsp;&nbsp;|&nbsp;&nbsp; Mode: **{selected_mode}**"
    )

    # --- Initialize persistent structures ---
    st.session_state.topic_mastery.setdefault(
        selected_topic,
        {b: 0 for b in df['bloom_level'].cat.categories}
    )
    st.session_state.score.setdefault(
        selected_topic,
        {b: {"correct": 0, "total": 0} for b in df['bloom_level'].cat.categories}
    )
    ...
    st.session_state.topic_mastery_status.setdefault(selected_topic, "Not started")

    # --- Mastery strip (compact progress overview) ---
    st.markdown("#### 📈 Mastery Snapshot")

    show_motivation_panel(selected_topic)

    # --- Start learning button ---
    if not st.session_state.get("started", False):
        st.info("Click the button below to begin.")
        start_label = "🚀 Start Practice" if selected_mode == "Practice" else "🧪 Start Test"
    ...
    if st.session_state.session_done:
        show_session_summary(selected_topic, selected_mode)
        return
        ...
        if st.session_state.review_mode and st.session_state.incorrect_review_queue:
            ...
            if st.session_state.review_mode:
                ...
                if st.session_state.freeze_bloom_count > 0:
                    ...
                    if not freeze_qs.empty:
                        q = freeze_qs.sample(1).iloc[0]
                        reason = f"🎯 Starting at {st.session_state.current_bloom} for {student_level} learner."
                        st.session_state.freeze_bloom_count -= 1
                    else:
    ...
    if st.session_state.current_question:
        display_question(
            pd.Series(st.session_state.current_question),
            selected_topic,
            selected_mode
        )
        ...
        if st.session_state.submitted:
            ...
            # Different session length by mode
            question_limit = 10 if selected_mode == "Practice" else 15
            if st.session_state.question_count >= question_limit and not st.session_state.review_mode:
                st.session_state.session_done = True
                st.rerun()
...
def display_question(q, topic, mode: str):
    st.info(st.session_state.current_reason)
    st.markdown(
        f"**Bloom:** `{q['bloom_level']}` &nbsp;|&nbsp; "
        f"**Question ID:** `{q['question_id']}` &nbsp;|&nbsp; "
        f"**Sub-Concept:** `{q.get('sub_concept', 'General')}`"
    )

    # Question stem (code-style)
    ...
    # Bookmark only in Practice mode
    if mode == "Practice":
        if bookmark_col.button("🔖 Bookmark", use_container_width=True):
            st.session_state.bookmarked.add(q['question_id'])
            st.success("Bookmarked for later review.")
    else:
        bookmark_col.markdown(" ")  # keep layout aligned
    ...
    if submit_col.button("✅ Submit", use_container_width=True) and not st.session_state.submitted:
        if choice is None:
            ...
            numeric_conf = CONFIDENCE_MAP.get(confidence, 3)
            handle_submission(q, choice, numeric_conf, topic, mode)
...
def handle_submission(q, choice, confidence, topic, mode: str):
    """
    Handles logic after a user submits an answer.
    Mode-aware:
      - Practice: explanations, mastery, bookmarks, review.
      - Test: no explanations until summary, still logs everything.
    """
    correct = q["correct_option"].strip().lower()
    is_correct = (choice == correct)
    bloom = q["bloom_level"]

    # Simple scoring: reward correctness + confidence
    ...
    if is_correct:
        ...
        if mode == "Practice":
            ...
            # Test mode – minimal feedback
            st.error("❌ Recorded. Answer will be reviewed in your summary.")
    ...
    # --- Practice mode explanations only ---
    if mode == "Practice":
        ...
        with st.expander("🔗 Learn More"):
            st.markdown(
                "Learn more about Java concepts at 👉 [W3Schools Java Tutorial]"
                "(https://www.w3schools.com/java/default.asp)",
                unsafe_allow_html=True
            )

    # --- Compute response time ---
    start_time = st.session_state.get("question_start_time", None)
    ...
    # --- Persist per-student CSV (append) ---
...
def show_session_summary(topic, mode):
    """
    Modernized, mode-aware session summary for Student Dashboard.
    Works with:
      - Practice Mode (detailed feedback, review wrong questions)
      - Test Mode (exam-like summary, no per-question feedback)
    Integrates with:
      - mastery tracking
      - Bloom level transitions
      - student CSV updates
    """
    st.markdown("## 🏁 Session Summary")

    # --- Filter logs for this session & topic ---
    session_log = [
        log for log in st.session_state.log
        if log['session_id'] == st.session_state.session_id
        and log['topic'] == topic
    ...
    if mode == "Test":
        st.markdown("### 🧪 Test Mode Summary")
        st.metric("Test Score", f"{correct}/{total}", f"{accuracy:.1f}%")
    else:
        st.markdown("### 📘 Practice Mode Summary")
    ...
    for log in session_log:
        b = log["bloom_level"]
        bloom_stats.setdefault(b, {"correct": 0, "total": 0})
        bloom_stats[b]["total"] += 1
        bloom_stats[b]["correct"] += int(log["is_correct"])

    cols = st.columns(len(bloom_stats))
    for idx, (bloom, stats) in enumerate(bloom_stats.items()):
        pct = (stats["correct"] / stats["total"]) * 100
        with cols[idx]:
            st.markdown(
                f"**{bloom}**<br>"
                f"{stats['correct']} / {stats['total']} correct<br>"
                f"({pct:.0f}%)",
                unsafe_allow_html=True
            )
            st.progress(int(pct))

    # --- Wrong Questions (Practice Mode only) ---
    incorrect_questions = [
        log['question_id'] for log in session_log if not log['is_correct']
    ]

    if mode == "Practice" and incorrect_questions:
    ...
    MASTERY_THRESHOLD = 80  # percent

    if total >= MIN_ITEMS_FOR_MASTERY and accuracy >= MASTERY_THRESHOLD:
        st.success(f"🎉 You achieved **mastery** for the topic **{topic}**!")
        st.balloons()
        st.session_state.topic_mastery_status[topic] = "Mastered"
    else:
        if st.session_state.topic_mastery_status.get(topic) != "Mastered":
            st.session_state.topic_mastery_status[topic] = "In progress"

    # ========= Level Promotion/Demotion (Practice Mode Only) =========
    if mode == "Practice" and not st.session_state.get("session_summary_done", False):
        LEVEL_ORDER = ["Beginner", "Intermediate", "Advanced"]
        current_level = st.session_state.student_level
        i = LEVEL_ORDER.index(current_level)

        if accuracy >= 70 and i < len(LEVEL_ORDER) - 1:
            st.session_state.student_level = LEVEL_ORDER[i + 1]
            st.success(f"🌟 Promoted to **{LEVEL_ORDER[i + 1]}**!")
        elif accuracy <= 50 and i > 0:
            st.session_state.student_level = LEVEL_ORDER[i - 1]
            st.info(f"⬇️ Adjusted to **{LEVEL_ORDER[i - 1]}** to reinforce skills.")

        # Update bloom level based on last question
        last_bloom = session_log[-1]["bloom_level"]
        st.session_state.current_bloom = last_bloom

        # Write back to student_list.csv
        try:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            csv_path = os.path.join(script_dir, "..", "data", "student_list.csv")
            df_students = pd.read_csv(csv_path, dtype=str)

            sid = str(st.session_state.user_id)
            mask = df_students["student_id"] == sid

            df_students.loc[mask, "level"] = st.session_state.student_level
            df_students.loc[mask, "current_bloom"] = st.session_state.current_bloom
        ...
        st.session_state.session_summary_done = True
...
def load_student_logs(student_id: str) -> pd.DataFrame:
    """
    Load all log rows for a given student from logs/student_<id>.csv.
    Returns empty DataFrame if no logs exist yet.
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.abspath(os.path.join(base_dir, ".."))
    logs_dir = os.path.join(project_root, "logs")
    csv_path = os.path.join(logs_dir, f"student_{student_id}.csv")
    ...
    try:
        ...
        st.warning(f"Could not load logs for student {student_id}: {e}")
        return pd.DataFrame()


def generate_report_card_pdf(student_id: str, student_name: str, df_logs: pd.DataFrame) -> bytes:
    """
    Generate a simple PDF report card summarizing mastery for a student.
    ...
    if df_logs.empty:
        ...
        c.drawString(72, y, f"Total Questions Answered: {total}")
        y -= 16
        ...
        c.drawString(72, y, "Per-Topic Performance:")
        y -= 18
        ...
        if y < 72:
            c.showPage()
            y = height - 72
        c.drawString(72, y, "Bloom-Level Performance:")
        y -= 18
        c.setFont("Helvetica", 11)
